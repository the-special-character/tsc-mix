...
export default function DesktopBentoDashboard() {
  const cardData = [
    ...
    { image: Learnimage, alt: "Learning", color: "bg-secondary3", href: "" },
  ];

  const [hoveredIndex, setHoveredIndex] = useState<number | null>(1);

  return (
    <div className="flex flex-col h-dvh px-container gap-4 md:gap-6 lg:gap-10 overflow-hidden">
      <header className="h-20">
        <TscLogo className="h-full py-4 md:py-3 lg:py-2" />
      </header>
      <main className="flex-1 flex flex-col gap-4 md:gap-6 lg:gap-10">
            ...
            {cardData?.map((card, cardIndex) => (
              <Link
                ...
                className={cn(
                  ...
                  {
                    "lg:scale-y-[1.20] lg:scale-x-[1.10] lg:translate-z-4":
