...
export default function TscContainer({ sections }: TscContainerProps) {
  ...
  const handleMouseEnter = (title: string) => {
    if (viewportSize === 'desktop') setActiveSection(title)
  }
        ...
        >
          ...
          >
            <motion.h2
              ...
              animate={{
                rotate: activeSection === section.title ? 0 : index % 2 === 0 ? -90 : 90,
              }}
              transition={{ 
                type: "spring", 
                stiffness: 100, 
          ...
          <AnimatePresence>
            {(activeSection === section.title || isNonDesktop) && (
              <motion.div
                initial={viewportSize === 'mobile' ? { opacity: 1 } : { opacity: 0 }}
                animate={{ opacity: 1 }}
                exit={{ opacity: 0 }}
                transition={{ 
                  type: "spring", 
                ...
                className={`absolute inset-0 flex items-center justify-center bg-primary ${section.bgActive}`}
              >
                <div className="flex flex-col text-black gap-4 bg-gradient-to-r from-secondary1 px-4 py-6 rounded-lg max-w-full sm:max-w-[80%] w-[90%]">
